...
public class BasicTraderStrategy implements ITraderStrategy {
    ...
    public List<Order> GenerateOrders(BigDecimal balance, List<Quote> quotesHistory, List<Quote> newQuotes, List<StockOwned> stocksOwned) {
        ...
        newQuotes.forEach(quote -> {
            ...
            if (ShouldBuy(balance, quote)) {
                var quantity = CalculateHowMuchToBuy(balance, quote, new BigDecimal("0.1"));
    ...
    private boolean ShouldBuy(BigDecimal balance, Quote quote) {
        ...
        return quote.ChangeInPercentage.compareTo(new BigDecimal("0.10").negate()) <= 0;
    }
    ...
    private boolean ShouldSell(Quote quote, List<StockOwned> stocksOwned) {
        ...
        return changeOfCurrentPriceToAverageBuyPricePercent.compareTo(new BigDecimal("0.10")) >= 0;
    }
